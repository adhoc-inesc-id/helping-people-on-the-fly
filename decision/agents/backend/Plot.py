...
from decision.agents.backend import RunStats
...
class Plot:
    ...
    def show(self, error_fill=True, error_fill_transparency=0.25, start_at_zero=False):
        if len(self._stats) > 0:
            if self._current_figure is not None: plt.close(self._current_figure)
            self._make_fig(error_fill, error_fill_transparency, start_at_zero=start_at_zero)
            self._current_figure.show()
            plt.close(self._current_figure)
            del self._current_figure
            self._current_figure = None

    def savefig(self, filename=None, error_fill=True, error_fill_transparency=0.25, start_at_zero=False):
        if len(self._stats) > 0:
    ...
    def _make_fig(self, error_fill=True, error_fill_transparency=0.25, show_legend=True, start_at_zero=False):

        x_ticks = (np.arange(self._x_ticks) + (1 if not start_at_zero else 0)) * self._x_tick_step
        self._current_figure, ax = plt.subplots(1)

        self._current_figure.set_size_inches(6, 4)
