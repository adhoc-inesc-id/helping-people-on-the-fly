...
def sizes_bar_plot(agent_names, alias, colors, width=0.25, confidence_level=0.95, show=False):

    X = np.arange(3)
    data = load_results(agent_names, confidence_level)
    ...
    if not show:
        ...
        plt.savefig(f"{RESOURCES_ROOT}/plots/gaips.png")
        plt.savefig(f"{RESOURCES_ROOT}/plots/pdf/gaips.pdf")
    else:
        plt.title(f"GAIPS")
        plt.show()
...
def load_results(agent_names, confidence_level):
    ...
    for task in range(1, 3):
        ...
        for agent_name in agent_names:
            ...
            for teammate in ("greedy", "random", "suboptimal"):
                ...
                files = yaaf.files(f"{RESOURCES_ROOT}/results/{domain}/{teammate}/small/task_{task}/{agent_name}")
                result_files = [file for file in files if ".npy" in file]
                results = []
                for file in result_files:
                    result = np.load(f"{RESOURCES_ROOT}/results/{domain}/{teammate}/small/task_{task}/{agent_name}/{file}")
                    if result.shape == (0,):
                        print(f"{RESOURCES_ROOT}/results/{domain}/{teammate}/small/task_{task}/{agent_name}/{file}")
                        logging.warning(f"Corrupt file {domain}/{teammate}/small/task_small/{agent_name}/{file}, moving to badfiles.")
                        yaaf.mkdir(f"{RESOURCES_ROOT}/badfiles/{domain}/{teammate}/small/task_{task}/{agent_name}")
                        shutil.move(
                            f"{RESOURCES_ROOT}/results/{domain}/{teammate}/small/task_{task}/{agent_name}/{file}",
                            f"{RESOURCES_ROOT}/badfiles/{domain}/{teammate}/small/task_{task}/{agent_name}/{file}"
                        )
                    else:
                        try:
...
if __name__ == '__main__':
    ...
    try:
        sizes_bar_plot(agents, agent_alias, colors, confidence_level=confidence_level, show=show)
